...
const GlowingDot = ({ color }: { color: string }) => {
  ...
  return (
    <View style={styles.dotContainer}>
      ...
      />
      <View
        style={[styles.coreDot, { backgroundColor: color, shadowColor: color }]}
      >
...
const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const chartConfig = React.useMemo(
    () => ({
      ...
      labelColor: (opacity = 1) =>
        isDarkMode ? `rgba(255, 255, 255, 1)` : `rgba(51, 51, 51, 1)`,
      propsForDots: {
        r: "4",
        strokeWidth: "2",
  ...
  const tooltipTop =
    tooltipPos !== null
      ? clamp(
          tooltipPos.y - tooltipHeight - 8,
          0,
          chartHeightPx - tooltipHeight,
        )
      : 0;

  const lastPointIndex = datasetValues.length - 1;
  const lastPointPos =
    lastPointIndex >= 0 ? getPointPosition(lastPointIndex) : null;
            ...
            {datasetValues.length > 1 ? (
              ...
              >
                {datasetValues.length === 0 && (
                  <Text style={{ color: subTextColor, marginTop: 80 }}>
